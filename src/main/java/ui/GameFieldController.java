...
public class GameFieldController {
    ...
    public void addStone(int x, int y,
        ...
        addNode(stone, x, y);
        engine.addBarUnit(x, y);

        int x1 = x * tmpWidth;
        int y1 = (fieldHeight - y) * tmpHeight;
        int offset = 1;
        if (magnetOnTop) {
            addMagnet(x1 + offset, y1 + offset, x1 + tmpWidth - offset, y1 + offset);
        }
        if (magnetOnBottom) {
            addMagnet(x1 + offset, y1 + tmpHeight - offset, x1 + tmpWidth - offset, y1 + tmpHeight - offset);
        }
        if (magnetOnRight) {
            addMagnet(x1 + tmpWidth - offset, y1 + offset, x1 + tmpWidth - offset, y1 + tmpHeight - offset);
        }
        if (magnetOnLeft) {
            addMagnet(x1 + offset, y1 + offset, x1 + offset, y1 + tmpHeight - offset);
        }
    ...
    public void addPlayer(int x, int y, Color color) {
        ...
        Image image = new Image("/ui/player.png");
        imageView.setImage(image);
        g.getChildren().addAll(player, imageView);
        playerList.add(g);
        addNode(g, x, y);
        engine.addPlayer(x, y, playerList.size());
    }
    ...
    public void move(int playerId, Movement movement) {
        ...
        switch (movement) {
            case UP:
                ...
                moveAmount = engine.movePlayer(playerId, movement, 1)[1];
                tt.setByY(-tmpHeight * moveAmount);
                break;
            case DOWN:
                tt = new TranslateTransition(duration, player);
                moveAmount = engine.movePlayer(playerId, movement, 1)[1];
            ...
            case LEFT:
                tt = new TranslateTransition(duration, player);
                moveAmount = engine.movePlayer(playerId, movement, 1)[0];
                tt.setByX(-tmpWidth * moveAmount);
                break;
            case RIGHT:
                tt = new TranslateTransition(duration, player);
                moveAmount = engine.movePlayer(playerId, movement, 1)[0];
        ...
        if (tt != null) {
            currentTransition.getChildren().add(tt);
        }
    }
